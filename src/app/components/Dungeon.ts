...
import { TILE_SUB_TYPE, TILE_TYPE } from '../../dungeon/constants';
  ...
  initGraphics(dungeonMap) {
    Object.keys(dungeonMap.tiles).map((k) => {
      ...
      if (tileMesh) {
        ...
        if (this.onClickTile) {
          tileMesh.onClick = (intersection) => this.onClickTile(tile, intersection);
        }
