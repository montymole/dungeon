...
export class UploadBin extends BaseController {
  ...
  async response() {
    return await new Promise((resolve, reject) => {
      ...
      busboy.on(
        'file',
        (fieldname, fileStream, name, encoding, contentType) => {
          ...
          fileStream.on('end', async () => {
            ...
            let result;
            if (exists) {
              result = await Binary.query().updateAndFetchById(exists.id, {
                name,
                filepath,
                contentType,
                size
              ...
              result = await Binary.query().insertAndFetch({
                name,
                filepath,
                contentType,
                size
